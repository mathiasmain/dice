...
model = keras.models.load_model('/content/drive/My Drive/Atual/Aulas/CCI210/aula08/CNN3camadas')
...
while(cap.isOpened()):
    ...
    gray = cv2.cvtColor(frame, cv2.COLO_BGR2GRAY)
    
    
    # Detect MultiScale detecta rostos de vários tamanhos com pelo menos
    # 64x64 de tamanho.
    
    faces = detect_bounding_box(frame, face_classifier) # Descobrir oq é faces
    # Daqui pra frente usa faces ou frame?
    
    str = whatEmotion(frame,model)
